...
describe("Monkey C Grammar", () => {
			...
			loadGrammar: (scopeName) => {
				if (scopeName === "source.mc") {
					return new Promise((resolve, reject) => {
						fs.readFile(
							path.join(__dirname, "../syntaxes/monkeyc.tmLanguage.json"),
							"utf-8",
							(err, contents) => {
	...
	const getTokens = (code: string) => {
		...
		let output: { text: string; scopes: string[] }[] = [];
	...
	it("should highlight primitive types as storage.type.builtin.mc", () => {
		...
		assert(
			asKeyword && asKeyword.scopes.includes("keyword.control.as.mc"),
			'Expected "as" to be keyword.control.as.mc',
		);
		assert(
			numberType && numberType.scopes.includes("storage.type.builtin.mc"),
			'Expected "Number" to be storage.type.builtin.mc',
		);
	});
	...
	it("should correctly scope Dictionary instantiation", () => {
		...
		assert(
			dictionary && dictionary.scopes.includes("storage.type.builtin.mc"),
			'Expected "Dictionary" to be storage.type.builtin.mc',
		);
	});
	...
	it("should correctly scope Array instantiation with generic types", () => {
		...
		assert(
			arrayWord && arrayWord.scopes.includes("storage.type.builtin.mc"),
			'Expected "Array" to be storage.type.builtin.mc',
		);
		assert(
			floatWord && floatWord.scopes.includes("storage.type.builtin.mc"),
			'Expected "Float" to be storage.type.builtin.mc',
		);
	});
	...
	it("should highlight typedef imports with as", () => {
		...
		assert(
			typedefWord && typedefWord.scopes.includes("keyword.control.typedef.mc"),
			'Expected "typedef" to be keyword.control.typedef.mc',
		);
		assert(
			numberType && numberType.scopes.includes("storage.type.builtin.mc"),
			'Expected "Number" to be storage.type.builtin.mc',
	...
	it("should correctly detect access modifiers for variables", () => {
		...
		assert(
			publicWord && publicWord.scopes.includes("storage.modifier.mc"),
			'Expected "public" to be storage.modifier.mc',
		);
	});
	...
	it("should correctly scope Symbol identifiers with reserved names like :width", () => {
		...
		assert(
			locX && locX.scopes.includes("constant.other.symbol.mc"),
			"Expected :locX to be constant.other.symbol.mc",
		);
		assert(
			width && width.scopes.includes("constant.other.symbol.mc"),
			"Expected :width to be constant.other.symbol.mc",
		);
	});
	...
	it("should properly scope Symbols inside Dictionary instantiation", () => {
		...
		assert(
			locX && locX.scopes.includes("constant.other.symbol.mc"),
			"Expected Dictionary key :locX to be constant.other.symbol.mc",
		);
	});
});
